...
namespace Identity.DAL.Core
{
    public class IdentityManager : IIdentityManager
    {
        ...
        public async Task<(bool Succeeded, string[] Errors)> CreateRoleAsync(AppRole role, IEnumerable<string> claims)
        {
            ...
            string[] invalidClaims = claims.Where(c => AppPermissions.GetPermissionByValue(c) == null).ToArray();
            ...
            {
                result = await this._roleManager.AddClaimAsync(role, new Claim(ClaimConstants.Permission, AppPermissions.GetPermissionByValue(claim)));
        ...
        public async Task<(bool Succeeded, string[] Errors)> UpdateRoleAsync(AppRole role, IEnumerable<string> claims)
        {
            ...
            if (claims != null)
            {
                ...
                if (claimsToAdd.Any())
                {
                    ...
                    {
                        result = await _roleManager.AddClaimAsync(role, new Claim(ClaimConstants.Permission, AppPermissions.GetPermissionByValue(claim)));
                        if (!result.Succeeded)
                            return (false, result.Errors.Select(e => e.Description).ToArray());
                    }
                }
